...
def d_eucl(x_temp, y_temp, z_temp, x2_temp, y2_temp, z2_temp):
    ...
    distance_temp3 = distance_temp1 + distance_temp2 + (z_temp - z2_temp) * (z_temp - z2_temp)
    distance_temp = math.sqrt(distance_temp3)
    return (distance_temp)

def d_eucl2(x_temp, y_temp,z_temp, x2_temp, y2_temp, z2_temp, goal_x, goal_y, goal_z):
    distance_temp1 = (x_temp - x2_temp) * (x_temp - x2_temp)
    distance_temp2 = (y_temp - y2_temp) * (y_temp - y2_temp)
    distance_temp = distance_temp1 + distance_temp2 + (z_temp - z2_temp) * (z_temp - z2_temp)
    distance_temp = math.sqrt(distance_temp)
    distance_temp3 = (x2_temp - goal_x) * (x2_temp - goal_x)
    distance_temp4 = (y2_temp - goal_y) * (y2_temp - goal_y)
    distance_temp5 = distance_temp3 + distance_temp4 + (z2_temp - goal_z) * (z2_temp - goal_z)
    distance_temp6 = math.sqrt(distance_temp5)
    distance_temp7 = distance_temp + 0.4*(distance_temp6)
    return (distance_temp7)
...
def search_path(closest_points, list_of_fields, iteration, goal_x, goal_y, goal_z):
    path = []
    for j in range(iteration):
        if (goal_x - 2) <= closest_points[0][0] <= (goal_x + 2) and (goal_y - 2) <= closest_points[0][1] <= (goal_y + 2) and (goal_z - 2) <= closest_points[0][2] <= (goal_z + 2) :
            ...
            for i in range(len(list_of_fields)):
                ...
                euclidean_points_dist = d_eucl2(closest_points[0][0], closest_points[0][1], closest_points[0][2], list_of_fields[i][0], list_of_fields[i][1], list_of_fields[i][2],goal_x, goal_y, goal_z)
                # print('i = ',i)
                ...
                if(i==0):
                    ...
                    temp_min_points.append((list_of_fields[i][0], list_of_fields[i][1], list_of_fields[i][2]))
                ...
                if i == (len(list_of_fields) - 1):
                    ...
                    for n in range(len(list_of_fields)):
                        if (n - counter) == (len(list_of_fields) - counter - 1):
                            break
                        if temp_min_points[0] == list_of_fields[n-1][0:3]:
                            list_of_fields.pop(n-1)
                            counter = counter + 1

                    path.append((temp_min_points[0][0], temp_min_points[0][1], temp_min_points[0][2]))
...
def search_min_distances(path, goal_coord):
    ...
    for i in range(len(path)):
        euclid_dist = d_eucl(path[i][0], path[i][1], path[i][2], goal_coord[0], goal_coord[1], goal_coord[2])
        ...
        if i == 0:
            ...
            if euclid_dist == path_distances[-1] and i+ 1 < len(path)-2:
                euclid_superset = d_eucl(path[i+1][0], path[i+1][1],path[i+1][2], goal_coord[0], goal_coord[1], goal_coord[2])
                if euclid_superset < euclid_dist:
                    optimal_path.pop(-1)
                    optimal_path.append(path[i])
                    path_distances.append(euclid_superset)
                if euclid_superset == euclid_dist and i + 2 < len(path)-2:
                    euclid_superset2 = d_eucl(path[i + 2][0], path[i + 2][1], path[i+2][2], goal_coord[0], goal_coord[1], goal_coord[2])
...
def path_smoothing(path, iter_number):
    ...
    x_list_x = [x for x, y, z in path]
    y_list_y = [y for x, y, z in path]
    z_list_z = [z for x, z, z in path]

    x_list = x_list_x.copy()
    y_list = y_list_y.copy()
    z_list = z_list_z.copy()
    x0 = x_list_x.copy()
    y0 = y_list_y.copy()
    z0 = z_list_z.copy()
    alpha = .3
    ...
    for i0 in range(0, iter_number):
        for i in range(1, len(path) - 1):
            ...
            z_list[i] = z_list[i] + alpha * (z_list[i - 1] + z_list[i + 1] - 2 * z_list[i])
            # part2
            x_list[i] = x_list[i] + beta * (x0[i] - x_list[i])
            y_list[i] = y_list[i] + beta * (y0[i] - y_list[i])
            z_list[i] = z_list[i] + beta * (z0[i] - z_list[i])

        if i0 == iter_number-1:
            new_path = list(zip(x_list,y_list, z_list))
...
while len(q) > 0:
    x_restriction = 150
    x_restriction_left = 1
    y_restriction = 150
    y_restriction_bottom = 1
    z_restriction_front = 1
    z_restriction_back = 150
...
print(new_f)
x, y, z = zip(*new_f)
# ax.scatter(x, y, z, c='green', marker='o', s = 50)  # You can change color and marker style

closest_points = []
closest_points.append(point_coordinates2)
path_coordinates = search_path(closest_points, new_f, 1000, point_coordinates[0], point_coordinates[1],point_coordinates[2])
optimal = search_min_distances(path_coordinates,point_coordinates)
smooth = path_smoothing(optimal,10)
# path_coordinates = search_min_distances(path_coordinates_1,point_coordinates2)
# print("Optmial path", path_coordinates)
# Extract the X, Y, and Z coordinates from the path
# x_path, y_path, z_path = zip(*path_coordinates)
# x_path, y_path, z_path = zip(*optimal)
x_path, y_path, z_path = zip(*smooth)
# Plot the path as a line
ax.scatter(x_path, y_path, z_path, c='lightgreen', marker='o', s = 50, alpha= 0.35, edgecolors='g')
ax.plot(x_path, y_path, z_path) #printing the line

# Set plot limits and labels (existing code)
# ... [your existing code for setting limits and labels]
ax.set_xlim([0, 150])
ax.set_ylim([0, 150])
ax.set_zlim([0, 150])
ax.set_xlabel('X')
